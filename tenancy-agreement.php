...
<div class="row">
<div class="col-sm-12">
<h3 class="page-title">Tenancy Agreement</h3>
<ul class="breadcrumb">
<li class="breadcrumb-item active">Kindly Perform any operations below</li>
</ul>
</div>
</div>
</div>
...
<style>
        ...
        .ticket-detail-icon.bg-danger-lights {
            color: #dc3545;
        }
        .ticket-detail-icon.bg-warning-lights {
            color: #ffc107;
        }
        .ticket-detail-icon.bg-purple-lights {
            color: #6f42c1;
        }
...
<div class="container mt-5">
        <div class="row">
            <div class="col-xxl-3 col-md-6">
                <div class="ticket-head-card">
                    <button type="button" class="btn btn-primary" id="sendToTenantBtn">Send to Tenant</button>
                </div>
            </div>
            <div class="col-xxl-3 col-md-6">
                <div class="ticket-head-card">
        ...
        <!-- Send to Tenant Form (hidden by default) -->
<form action="" method='post' class="form-container">
    <div id="sendToTenantForm" class="hidden action-content">
        <h4>Send to Tenant</h4>
        <br>
        ...
        <label for="businessDropdown">Select Business</label>
                <select class="form-control" id="businessDropdown" name="recipientEmail" required>
                    <option value="" disabled selected>Select a business</option>
                    <?php if ($fetchallclients->num_rows > 0): ?>
                        <?php while($row = $fetchallclients->fetch_assoc()): ?>
                            <option value="<?php echo htmlspecialchars($row['contact_person_email']); ?>"><?php echo htmlspecialchars($row['business_name']); ?></option>
                        <?php endwhile; ?>
                    <?php else: ?>
        ...
        <label for="title">Title</label>
                <input type="text" class="form-control" id="title" name="messageTitle" required>
                <input type="hidden" value="<?php echo $username; ?>" name="senderName">
                <input type="hidden" value="<?php echo $useremail; ?>" name="senderEmail">
                <input type="hidden" value="<?php echo $useremail; ?>" name="ccEmail">
                <div class="invalid-feedback">Please enter a title.</div>
        </div>
        <br>
        <div class="form-group">
        <label for="body">Body</label>
                <textarea class="form-control" id="body" name="messageBody" rows="4" required></textarea>
                <div class="invalid-feedback">Please enter the body of the message.</div>
        </div>
        <br>
        <button name="sendtenancy" type="submit" class="btn btn-primary">Send</button>
    </div>
    <br>
</form>

        <!-- Download Options (hidden by default) -->
        <div id="downloadOptions" class="hidden action-content">
            <h4>Download Options</h4>
            <p>Select file format to download.</p>
            <a id="pdfDownloadLink" href="documents/Tenant-Assessment-Application-Form.pdf" download="Tenant-Assessment-Application-Form.pdf">
                <button type="button" class="btn btn-primary">Download PDF</button>
            </a>
            <a id="docxDownloadLink" href="documents/Tenant-Assessment-Application-Form.docx" download="Tenant-Assessment-Application-Form.docx">
                <button type="button" class="btn btn-secondary">Download DOCX</button>
            </a>
        </div>
    </div>

    <script>
        document.getElementById('sendToTenantBtn').addEventListener('click', function() {
            document.getElementById('sendToTenantForm').classList.toggle('hidden');
        ...
        document.getElementById('downloadBtn').addEventListener('click', function() {
            ...
            document.getElementById('sendToTenantForm').classList.add('hidden');
        });
